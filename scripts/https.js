...
Usage:
  ...
  epic https install-ca --remote user@host [--nss auto|always|never] [--name "mkcert epicstory (host)"]
...
function takeFlagValue(args, flag) {
  ...
  if (!next || next.startsWith("-")) return { value: undefined, rest: args.filter((_, i) => i !== idx) };
...
async function ensureMkcert() {
  try {
    ...
    console.error("❌ mkcert not found. Install it first (see docs/dev-https.md).");
    process.exit(1);
  }
}
...
async function genCerts(args) {
  ...
  await run("mkcert", ["-cert-file", certPath, "-key-file", keyPath, domain], { cwd: repoRoot });
  console.log(`✅ Wrote:\n- ${certPath}\n- ${keyPath}`);
...
async function installCa(args) {
  ...
  const r2 = takeFlagValueWithDefault(r1.rest, "--nss", "auto");
  const nssMode = r2.value;
  const r3 = takeFlagValueWithDefault(r2.rest, "--name", "mkcert epicstory (host)");
...
`;
  ...
  await run("ssh", [remote, "bash", "-lc", remoteScript]);
