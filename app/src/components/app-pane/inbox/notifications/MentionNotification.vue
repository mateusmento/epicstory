...
<template>
  <div class="flex:col-md">
    <div class="flex:row-md flex:center-y text-sm text-secondary-foreground font-dmSans">
      @ mentioned you in <span class="text-foreground font-semibold">{{ channelName }}</span>
    </div>
    <div v-if="payload.sender" class="flex:row-md flex:center-y flex-1">
      ...
      <div class="flex:col-md flex-1 min-w-0">
        ...
        <div class="text-sm text-secondary-foreground font-lato text-ellipsis overflow-hidden whitespace-nowrap">
          {{ payload.message }}
        </div>
      </div>
    </div>
