...
const meta = {
  title: "Presentational/Layout/DrawerPaneContent",
  component: DrawerPaneContent,
  decorators: [
    (story) => ({ render: () => <StoryContainer class="w-[460px] h-[560px]">{h(story())}</StoryContainer> }),
  ],
} satisfies Meta<typeof DrawerPaneContent>;
    ...
    template: `
      <DrawerPane view="details-pane" v-model:open="open" class="w-80 bg-card">
        <DrawerPaneContent content="channel">
          <div class="p-4 text-sm">Channel details panel content</div>
        </DrawerPaneContent>
      </DrawerPane>
    `,
  }),
