...
/**
 * Same-channel quotes, or cross-channel quotes of an issue comment channel
 * (when `isIssueCommentChannel` is true for the quoted message's channel).
 */
export function assertQuotedParentMessageAllowed(
  target: Pick<Message, 'channelId'> | null | undefined,
  channelId: number,
  options?: { isIssueCommentChannel?: boolean },
): asserts target is Pick<Message, 'channelId'> {
  if (target == null) {
    throw new QuotedContentRuleError('Quoted message not found');
  }
  if (target.channelId === channelId) return;
  if (options?.isIssueCommentChannel) return;
  throw new QuotedContentRuleError(
    'Quoted message must belong to this channel',
  );
}
