...
**Snapshot (code in repo vs docs):** Tasks **03–06** MVP code paths are implemented (installation + user OAuth, Redis/memory cache, HTTP backoff, repo links, issue branch/PR + webhooks). **Task 02** is operational (register app per environment). **Task 07** (chat/palette) and **06** optional PR lifecycle comments remain deferred. **03** acceptance tests for OAuth `state`/duplicate callback are still open.
