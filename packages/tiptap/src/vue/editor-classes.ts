...
/**
 * Code blocks rendered as the card node view use `div.epic-code-card-pre` (see app
 * `TiptapCodeBlockCardNodeView.vue`), not `<pre>`. These utilities only style plain
 * TipTap `pre > code` output (fallback / pasted HTML), and `:not(.epic-code-card-pre)`
 * avoids clashing if a `pre` ever reused that class.
 */
const EPIC_STORY_PLAIN_CODEBLOCK_PRE_COMPOSER =
  "[&_pre:not(.epic-code-card-pre)]:my-2 [&_pre:not(.epic-code-card-pre)]:rounded-md [&_pre:not(.epic-code-card-pre)]:bg-zinc-100 [&_pre:not(.epic-code-card-pre)]:p-3 [&_pre:not(.epic-code-card-pre)]:overflow-x-auto [&_pre:not(.epic-code-card-pre)]:text-left " +
  "[&_pre:not(.epic-code-card-pre)_code]:font-mono [&_pre:not(.epic-code-card-pre)_code]:text-[0.8125rem] [&_pre:not(.epic-code-card-pre)_code]:text-foreground " +
  "[&_pre:not(.epic-code-card-pre)_code.hljs]:bg-transparent ";

const EPIC_STORY_PLAIN_CODEBLOCK_PRE_READONLY =
  "[&_pre:not(.epic-code-card-pre)]:my-2 [&_pre:not(.epic-code-card-pre)]:rounded-md [&_pre:not(.epic-code-card-pre)]:bg-muted [&_pre:not(.epic-code-card-pre)]:p-3 [&_pre:not(.epic-code-card-pre)]:overflow-x-auto " +
...
export const EPIC_STORY_COMPOSER_EDITOR_CLASS =
  ...
  EPIC_STORY_PLAIN_CODEBLOCK_PRE_COMPOSER +
  EPIC_STORY_INLINE_CODE +
...
export const EPIC_STORY_READONLY_MESSAGE_CLASS =
  ...
  "[&_blockquote]:bg-muted/30 [&_blockquote]:rounded-r-md " +
  EPIC_STORY_PLAIN_CODEBLOCK_PRE_READONLY +
  EPIC_STORY_TASK_LIST_CLASSES +
  " " +
  EPIC_STORY_TABLE_CLASSES +
  " " +
